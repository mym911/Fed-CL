...
class BaseLearner(object):  # Herding贪心挑样本
    def __init__(self, args):
        super().__init__()
        self._cur_task = -1  # 当前任务编号,从-1起,进入首任务前会自增到0
    ...
    def build_rehearsal_memory(self, data_manager, per_class):  # per_class每个类别要保留的样本数
        if self._fixed_memory:  # 统一的总内存上限下统一重建记忆库.
            self._construct_exemplar_unified(data_manager, per_class)
        else:  # 非固定内存
            self._reduce_exemplar(data_manager, per_class)  # 把旧类已保存的样本裁剪到每类m个,并用新网络重算旧类的均值.
            self._construct_exemplar(data_manager, per_class)  # 更新类的数据
    ...
    def _construct_exemplar_unified(self, data_manager, m):  # 固定内存模式下（每个类别只保留m个样本）
        logging.info(
            "Constructing exemplars for new classes...({} per classes)".format(m)
        )
        _class_means = np.zeros((self._total_classes, self.feature_dim))  # 按当前总类别数×特征维度建_class_means零矩阵,用来存每类的原型向量.

        # Calculate the means of old classes with newly trained network
        for class_idx in range(self._known_classes):  # 从已有记忆库取出旧类样本,均值重计算（旧类原型与新表型空间对齐）
            mask = np.where(self._targets_memory == class_idx)[0]  # 取出每个旧类的样本
            ...
            vectors, _ = self._extract_vectors(class_loader)  # 收集得到的特征值.
            vectors = (vectors.T / (np.linalg.norm(vectors.T, axis=0) + EPSILON)).T  # 归一化.vectors形状是[N,D],N个样本,每个样本D维特征.
            mean = np.mean(vectors, axis=0)  # 对每一列求平均,得到一个D维的平均特征向量.
            mean = mean / np.linalg.norm(mean)  # 用向量除以其范数得到长度为1的方向向量（只保留方向信息）

            _class_means[class_idx, :] = mean

        # Construct exemplars for new classes and calculate the means
        for class_idx in range(self._known_classes, self._total_classes):  # 遍历所有新类
            ...
            for k in range(1, m + 1):
                S = np.sum(
                    exemplar_vectors, axis=0
                )  # 把已选样本的特征向量按列相加,得到它们的向量和s
                mu_p = (vectors + S) / k  # 把每个候选样本加入当前已选集合后得到的近似均值
                i = np.argmin(np.sqrt(np.sum((class_mean - mu_p) ** 2, axis=1)))  # 取当前代表集均值最逼近整体类均值的样本索引.
