...
class DataManager(object):  # 增量学习的数据调度器DataManager
    ...
    def get_dataset(
    ...
    ):  # 根据要求,筛选数据并打包成Dataset输入给DataLoader.
        if source == "train":
        ...
        for idx in indices:
            if m_rate is None:  # 如果没有给缺失率.
                ...
                )  # 按m_rate随机丢弃一部分,只保留（1-m_rate)比例.
    ...
    ):  # 与get_dataset多在每个类别内再切一份验证集.val_samples_per_class作为验证集.
        if source == "train":
            x, y = self._train_data, self._train_targets
        elif source == "test":
            x, y = self._test_data, self._test_targets
    ...
    def _setup_data(self, dataset_name, shuffle, seed):  # 把原始数据准备成增量学习要用的统一格式.
    ...
    def _select_rmm(self, x, y, low_range, high_range, m_rate):  # randint有放回抽样
