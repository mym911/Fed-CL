...
from timm.models.registry import register_model  # 注册模型
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
       ...
       ###############################################################################
        ...
        self.head = nn.Sequential(nn.Linear(self.embed_dim, 512),
                                  nn.ReLU(),
                                  nn.Linear(512, self.num_classes))

        # another classifier head for local training
        self.head_local = self.head = nn.Sequential(nn.Linear(self.embed_dim, 512),
                                  nn.ReLU(),
                                  nn.Linear(512, self.num_classes))
    ...
    def init_pos_embedding(self):
        embed_len = self.patch_embed.num_patches if self.no_embed_class else self.patch_embed.num_patches + self.num_prefix_tokens
        if self.prompt_length is not None and self.pool_size is not None and self.prompt_pool:
            embed_len += self.prompt_length * self.top_k
        # position 信息
    ...
    def forward_server(self,x, cls_features=None,train=False,train_global_prompt=False,prompt=None,head=None):
        res = self.forward_features_l2p(x, cls_features=cls_features, train=train)
        res = self.forward_head_l2p(res)
        return res
